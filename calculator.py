...
def love():
    class Calculator:
        def __init__(self, master):
            ...
            # screen widget
            self.screen = Text(master, state='disabled', width=60, height=3, background="#fcfcec",
                               foreground="#cb464e", font=("times", 12, "bold"))
        ...
        def createButton(self, val, write=True, width=7):
            return Button(self.master, text=val, command=lambda: self.click(val, write), width=width,
                          background="#4b7fa4", foreground="#fcfcec", font=("times", 20))
        ...
        def insert_screen(self, value, newline=False):
            ...
            # recorded every value inserted in screen
            self.equation += str(value)
            self.screen.configure(state='disabled')

    root = Tk()
    _my_gui = Calculator(root)
    root.mainloop()
